...
impl MiniLsm {
    pub fn close(&self) -> Result<()> {
        ...
        // WAL not enabled: flush all memtables before closing
...
impl LsmStorageInner {
    ...
    pub fn force_freeze_memtable(&self, state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        let new_memtable = if self.options.enable_wal {
            MemTable::create_with_wal(new_id, self.path_of_wal(new_id))?
        } else {
            MemTable::create(new_id)
        };
        // Capture reference to the current memtable before freezing, so we can sync its WAL
        // even if the flush thread concurrently removes it from imm_memtables.
        let old_memtable = Arc::clone(&self.state.read().memtable);
        self.freeze_with_memtbale(new_memtable);
