...
impl LeveledCompactionController {
    ...
    /// Find SSTs in the target level that overlap with the given SSTs' key ranges.
    ///
    /// [IMPLEMENTED VIA KIMI CODE]
    ...
    /// Generates a compaction task for leveled compaction (RocksDB Leveled Compaction).
    ///
    /// [IMPLEMENTED VIA KIMI CODE]
    pub fn generate_compaction_task(
        ...
        if snapshot.l0_sstables.len() >= self.options.level0_file_num_compaction_trigger {
            eprintln!("flush L0 SST to base level {}", base_level);
    ...
    /// Apply the compaction result to the LSM state.
    ///
    /// [IMPLEMENTED VIA KIMI CODE]
